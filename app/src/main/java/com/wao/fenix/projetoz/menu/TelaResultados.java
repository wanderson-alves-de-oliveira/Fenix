...
public class TelaResultados extends AppCompatActivity implements GLSurfaceView.Renderer, GLSurfaceView.OnTouchListener {
    ...
    private GL10 gl2;
    private boolean fasecarregada = false;
    ...
    public TelaResultados(Context context, AssetManager asset) throws IOException {
    ...
    private void validarToque(MotionEvent event) {
        ...
        for (Objeto3d obj : bolhas) {
            double start = calculoarDistancia(obj, bolhaRef.getPosition());
             if (start <= 0.015 || iniciaraProxima) {
                sel=true;
